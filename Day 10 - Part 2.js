//Create an object for each bot and parse intitial value
var string = `bot 119 gives low to bot 18 and high to bot 3
...
function checkTypeExists (number, type) {

  for (var i = 0; i < type.length; i++) {
    if (type[i].number === number) {
      return {
        typeExists: true,
        index: i
      }
...
function findCommand (botNumber) {
  for (var i = 0; i < split.length; i++) {
    var temp = split[i].split(" ");
    if (temp[0] === "bot" && temp[1] === botNumber) {
      return {
        command: temp,
        lowRecipientType: temp[5],
        lowRecipient: temp[6],
        highRecipientType: temp[10],
        highRecipient: temp[11]
      };
    }
  }
}

function createNewType (typeNumber, chip, type) {
   var temp = {
      "number": typeNumber,
      "value": []
      }
    temp.value.push(chip);
    type.push(temp);
}


function addChip (receiverIndex, chip, type) {
  type[receiverIndex].value.push(chip);
}

function removeChip (giverIndex, chip) {
  var index = bot[giverIndex].value.indexOf(chip);
      if (index > -1) {
        bot[giverIndex].value.splice(index, 1);
      } else {
        console.log("Error! Chip not found");
      }
}

function executeCommand (chip, typeNumber, giverIndex, type) {
  var receiver = checkTypeExists(typeNumber, type);
  if (receiver.typeExists === false) {
   createNewType(typeNumber, chip, type);
   removeChip(giverIndex, chip);
  } else if (receiver.typeExists === true) {
   addChip(receiver.index, chip, type);
   removeChip(giverIndex, chip);
  }
}
...
function outputResult () {
  ...
  for (var i = 0; i < output.length;i++) {
    if (output[i].number === "2" || output[i].number === "1" || output[i].number === "0") {
    result *= parseInt(output[i].value);
    }
  }
  console.log("The sum of output 0, 1, and 2 is " + result);
}

//*** MAIN PROGRAM ***//

//Part 1: Handle "Value" Commands: create initial bot and update existing ones
for (var i = 0; i < split.length; i++) {
  var temp = split[i].split(" ");
  ...
  if (temp[0] === "value") {
    ...
    var botCheck = checkTypeExists(temp[5], bot);
    if (botCheck.typeExists === true) {
      bot[botCheck.index].value.push(parseInt(temp[1]));
    } else {
      createNewType(temp[5], parseInt(temp[1]), bot);
    }
  }
}
//console.log(bot);

//Part 2: Find bot with 2 chips and execute command

for (var x = 0; x < 15; x++) {
  for (var i = 0; i < bot.length; i++) {
  var currentBot = bot[i];
  if (currentBot.value.length > 1) {
    if (bot[i].value.includes(17)) {
      if (bot[i].value.includes(61)) {
        console.log("Found you! The bot comparing 17 and 61 chips is #" + bot[i].number);
      }
    }
    var chips = compareChips(bot[i].value[0], bot[i].value[1]);
    var currentCommand = findCommand(currentBot.number);
    executeCommand(chips.low, currentCommand.lowRecipient, i, eval(currentCommand.lowRecipientType));

    executeCommand(chips.high, currentCommand.highRecipient, i, eval(currentCommand.highRecipientType));
  }
}
}
outputResult();
